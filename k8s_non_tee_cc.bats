...
@test "Test trust storage" {
	...
	for IMAGE in ${EXAMPLE_IMAGE_LISTS[@]}; do
		pod_config="$TEST_COCO_PATH/../fixtures/sts-lvm.yaml"
		...
		if ! kubernetes_wait_cc_pod_be_ready "${POD_NAME}-0"; then
			kubectl delete -f $TEST_COCO_PATH/../fixtures/sts-lvm.yaml
		...
		checkout_pod_yaml "$TEST_COCO_PATH/../fixtures/sts-lvm-snap.yaml" $IMAGE
		kubectl apply -f $TEST_COCO_PATH/../fixtures/sts-lvm-snap.yaml
		eval $(parse_yaml $TEST_COCO_PATH/../fixtures/sts-lvm-snap.yaml "snap_")
		...
		if ! kubernetes_wait_cc_pod_be_ready "${SNAP_POD_NAME}-0"; then
			...
			kubectl delete -f $TEST_COCO_PATH/../fixtures/sts-lvm-snap.yaml
...
@test "Test signed image" {
	for IMAGE in ${EXAMPLE_IMAGE_LISTS[@]}; do
		...
		checkout_pod_yaml "$TEST_COCO_PATH/../fixtures/pod-config.yaml" $IMAGE
		pod_config="$TEST_COCO_PATH/../fixtures/pod-config.yaml"
		create_test_pod $pod_config
		kubectl get pods
		eval $(parse_yaml $pod_config "_")
...
@test "Test encrypted image" {
	...
	# $TEST_COCO_PATH/../run/losetup-crt.sh $ROOTFS_IMAGE_PATH c
	for IMAGE in ${EXAMPLE_IMAGE_LISTS[@]}; do
		checkout_pod_yaml "$TEST_COCO_PATH/../fixtures/k8s-cc-ssh.yaml" $IMAGE
